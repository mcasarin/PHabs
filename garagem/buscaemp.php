<?php
	...
	$empresa = $_POST['palavra'];
	
	//Pesquisar no banco de dados nome do curso referente a palavra digitada pelo usuário
	$sqlb = "SELECT * FROM autoriza WHERE empresa LIKE '%$empresa%'";
	$resultado_empresa = mysqli_query($conn, $sqlb);
	
	if(mysqli_num_rows($resultado_empresa) <= 0){
		echo "Nenhum empresa encontrado...";
	}else{
        echo "<table class=\"table table-sm table-striped table-bordered table-hover\">
                    ...
                    <tbody>";
		while($rows = mysqli_fetch_assoc($resultado_empresa)){
			$id = $rows['id_aut'];
