...
$buscanome = filter_input(INPUT_POST, 'palavra', FILTER_SANITIZE_STRING);
$sql = "SELECT * FROM corr_entrada WHERE nome like '%$buscanome%' ORDER BY dataInfo DESC limit 5;";
...
if(($resultado_busca) and ($resultado_busca->num_rows != 0)){
    while($row = $resultado_busca->fetch_array(MYSQLI_ASSOC)){
        echo "<tr><td>".$row['nome']."</td><td>".$row['registroar']."</td><td>".$row['dataInfo']."</td><td>".$row['conjunto']."</td><td>".$row['obs']."</td><td>".$row['disc']."</td><td>";
