...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	$fileName = $_FILES['fileCadCartaoUsuarioLote']['name'];
	$fileTmpName = $_FILES['fileCadCartaoUsuarioLote']['tmp_name'];
	$fileSize = $_FILES['fileCadCartaoUsuarioLote']['size'];
	//$fileError = $_FILES['fileCadCartaoUsuarioLote']['error'];
	$fileType = $_FILES['fileCadCartaoUsuarioLote']['type'];
	...
	while(!feof($arquivo)){
		...
		If ($dados[0] != 'nw' && $dados[0] != 'fim' && !empty($linha) && $dados[0] != ''){ 
			...
			{
				$linhap++; // inserido para saber em qual linha do excel deu erro no carto
				echo "coluna codigo composta com mais de 5 números<br>Na linha $linhap<br>";
				break;
			}
			//*/
			//-------------------------------------------------------------------------------------------------------
		  ...
		  if (is_numeric($dados[3]) ) 
		  {
			...
			if($sqle->num_rows > 0){
				while($l = $sqle->fetch_array(MYSQLI_ASSOC)){
					...
					$verifcard =  "select * from cartoes where sequencia LIKE '".$dados[2]."' or cartao = '".$junta."'  ";
					$verifcarde = $conn->query($verifcard);
					if($verifcarde->num_rows == 0){
						...
						$status = "<font color = \"blue\"> CARTÃO JA CADASTRADO </font>";
						break;
					}
				}
			} else {
				$emp = "<font color = \"#000000\"> EMPRESA COM </font> ".$dados[3]." <font color = \"#000000\">  NO COMEÇO NÃO LOCALIZADA </font>  ";
				...
				if($verifemp->num_rows > 0)
				{
					...
					}//fecha while que pega a empresa que come�a com letras
				}
				else
				{
					$emp = "<font color = \"#000000\"> EMPRESA COM </font> ".$dados[3]." <font color = \"#000000\">  NO COMEÇO NÃO LOCALIZADA </font>  ";
					$status = "<font color = \"red\"> CARTÃO NÃO FOI CADASTRADO </font>";
				}
		  }//fecando else que verifica se empresa come�a com numero ou letras	
		  //---------------------------------------------------------------------
			if( ($cor%2) == 0)
			{
 ...
 fclose($arquivo); // Fecha arquivo aberto
	...
	if ($cadastro <= 1){
		if ($cadastro == 0)
		{	 
			echo "<br><div class='alert alert-danger'>Não foi cadastrado nenhum cartão</div>";
		}
		else
		{
			echo "<br><div class='alert alert-success'>$cadastro cartão cadastrado</div>";
		}
	} else {
		echo "<br><div class='alert alert-success'>$cadastro cartões cadastrados</div>";
	}
	unlink($path);
} // if extensão
